...
export function HudRecommendation({ fingers, showExtras = true }: HudRecommendationProps) {
        ...
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-4">
            <div className="glass-card p-4">
              <p className="text-xs font-heading uppercase tracking-[0.12em] text-green-400/80 mb-2">Pro Tips</p>
            ...
            <div className="glass-card p-4">
              <p className="text-xs font-heading uppercase tracking-[0.12em] text-red-400/80 mb-2">Errores Comunes</p>
              <div className="space-y-1.5">
          ...
          {layout.transitionEs && (
            <div className="glass-card p-4 mb-4 border-blue-500/10">
              <div className="flex items-start gap-2">
                ...
                <div>
                  <p className="text-xs font-heading uppercase tracking-[0.12em] text-blue-400/80 mb-1">Guía de Transición</p>
              ...
              >
                <div className="grid grid-cols-3 gap-2">
                  {FINGER_OPTIONS.map((f) => {
                    ...
                    return (
                      <div key={f} className={cn('rounded-xl p-2 border transition-all', isActive ? 'bg-red-500/5 border-red-500/25 shadow-[0_0_15px_rgba(239,68,68,0.08)]' : 'bg-white/[0.02] border-white/[0.05] opacity-60')}>
                        <p className="text-[10px] font-heading font-bold text-center text-white mb-1">{f} Dedos</p>
                        <div className="space-y-0.5 text-center">
                          <p className="text-[9px] text-slate-500 font-body">{profile.capabilities.simultaneousActions} acciones</p>
                          <p className="text-[9px] text-slate-500 font-body">{profile.competitiveLabelEs}</p>
                          <p className="text-[9px] text-slate-600 font-body">~{l.adaptationDays}d adaptación</p>
                        </div>
                      </div>
                    );
                  })}
          ...
          <button
            ...
            className={cn('w-full min-h-[44px] px-4 py-3 rounded-xl text-sm font-ui font-bold transition-all', copied ? 'bg-green-500/10 border border-green-500/30 text-green-400' : 'bg-white/[0.04] border border-white/10 text-slate-300 hover:bg-white/[0.06] hover:border-white/15')}
          >
